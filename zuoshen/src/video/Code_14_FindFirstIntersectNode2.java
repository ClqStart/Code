...
//有环的情况：单列表只能环在最后面
public class Code_14_FindFirstIntersectNode2 {
    private static Node getIntersectNode(Node head1, Node head2) {
        ...
        System.out.print(loop1==null?null:loop1.value+" ");
        System.out.print(loop2==null?null:loop2.value+" ");
    ...
    private static Node getLoopNode(Node head1) {
        ...
        //有环指针相遇，快指针从头来开始和慢指针相同步调，两指针相遇就是环的入口点
        fast = head1;
        while (fast != slow) {
            fast = fast.next;
            slow = slow.next;
    ...
    private static Node noLoop(Node head1, Node head2) {
        //获取两个列表差值
        //判断最后一个节点是否相同判断是否相交
        //相交时：长的列表先走差值数量，再和短的列表同时走肯定达到第一个相交的点
        int n = 0;
        Node cur1 = head1;
        Node cur2 = head2;
    ...
    /**
     * 有环三中情况
     * 两个环不想交，相交于同一个环的一个口，相较于同一个环的两个口(随便返回其中一个口)
     * @param head1
     * @param loop1
     * @param head2
     * @param loop2
