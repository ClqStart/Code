...
/**
 * 给定一个Job类型的数组jobarr,表示所有岗位，每个岗位可以提供任意份工作，选工作的标准是
 * 不超过自生能力的情况下，选择报酬最高的岗位
 * 给定一个数组int类型的表示所有人的能力；
 * 返回int的类型的数组，表示每个人按标准选择工作后获得最高的报酬
 */
...
public class code_15_ChooseWork {
    ...
    public static class JobComparetor implements Comparator<Job> {
    ...
    public static int[] getMoneys(Job[] job, int[] ability) {
        Arrays.sort(job, new JobComparetor());
        TreeMap<Integer, Integer> map = new TreeMap<>();
        map.put(job[0].hard, job[0].money);
        for (int i = 1; i < job.length; i++) {
            //难度上升，钱上升
            if (job[i].hard != job[i - 1].hard && job[i].money > job[i - 1].money) {
                map.put(job[i].hard, job[i].money);
            }
        }
        int[] salary = new int[ability.length];
        for (int i = 0; i < ability.length; i++) {
            Integer key = map.floorKey(ability[i]);
            salary[i] = key != null ? map.get(key) : 0;
        }
       return salary;
    }
